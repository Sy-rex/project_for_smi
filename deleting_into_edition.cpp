...
#include "ui_deleting_into_edition.h"
...
deleting_into_edition::deleting_into_edition(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::deleting_into_edition)
{
    ui->setupUi(this);
    ...
    QLabel *label1 = new QLabel("DELETING FROM EDITION", this);
    ...
    connect(menuButton, &QPushButton::clicked, this, &deleting_into_edition::openMainWindow);
...
deleting_into_edition::~deleting_into_edition()
{
    delete ui;
}

void deleting_into_edition::paintEvent(QPaintEvent *event)
{
...
void deleting_into_edition::openMainWindow()
{
...
void deleting_into_edition::deleteRecord(QModelIndex index) {
    ...
    if (msgBox.exec() == QMessageBox::Yes) {
        QSqlQuery query;
        query.prepare("UPDATE authors SET edition_id = NULL WHERE edition_id = :id");
        query.bindValue(":id", id);

        if (query.exec()) {
            qDebug() << "Successfully updated related authors.";
            query.prepare("DELETE FROM edition WHERE id = :id");
            query.bindValue(":id", id);
            if (query.exec()) {
                ...
                refreshQuery.prepare("SELECT id, name AS \"Название\", main_editor AS \"Редактор\", year_of_creation AS \"Дата основания\", rating AS \"Рейтинг\" FROM edition");
                refreshQuery.exec();
...
void deleting_into_edition::customMenuRequested(QPoint pos) {
