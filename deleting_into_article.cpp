...
#include "ui_deleting_into_article.h"
...
deleting_into_article::deleting_into_article(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::deleting_into_article)
{
    ui->setupUi(this);
    ...
    QLabel *label1 = new QLabel("DELETING FROM ARTICLE", this);
    ...
    connect(menuButton, &QPushButton::clicked, this, &deleting_into_article::openMainWindow);
...
deleting_into_article::~deleting_into_article()
{
    delete ui;
}

void deleting_into_article::paintEvent(QPaintEvent *event)
{
...
void deleting_into_article::openMainWindow()
{
...
void deleting_into_article::deleteRecord(QModelIndex index) {
    ...
    if (msgBox.exec() == QMessageBox::Yes) {
        ...
        query.prepare("DELETE FROM article_authors WHERE article_id = :id");
        ...
        if (query.exec()) {
            ...
            refreshQuery.prepare("SELECT article.id, journal.name AS \"Журнал\", article.name AS \"Статья\", article.rating AS \"Рейтинг\", article.description AS \"Описание\", article.price AS \"Стоимость\", article.release_date AS \"Опубликовано\" "
                                 "FROM article "
                                 "LEFT JOIN journal ON article.journal_id = journal.id");
            refreshQuery.exec();
            model->setQuery(refreshQuery);
        } else {
            qDebug() << "Error deleting from article:" << query.lastError();
            showErrorMessageBox("Не удалось удалить запись из таблицы article.");
        }
    }
}

void deleting_into_article::showErrorMessageBox(const QString &message) {
...
void deleting_into_article::customMenuRequested(QPoint pos) {
