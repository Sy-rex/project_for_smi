...
updating_article::updating_article(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::updating_article)
{
    ui->setupUi(this);
    ...
    connect(menuButton, &QPushButton::clicked, this, &updating_article::openMainWindow);
    ...
    connect(imageButton2, &QPushButton::clicked, this, [this]() {
        updateArticleEntry(article, comboBox, lineEdit, lineEdit2, lineEdit3, lineEdit4, lineEdit5);
    });
...
void updating_article::paintEvent(QPaintEvent *event)
{
...
void updating_article::openMainWindow()
{
...
void updating_article::loadComboBoxData(QComboBox *comboBox) {
...
void updating_article::updateArticleEntry(int article, QComboBox *comboBox, QLineEdit *lineEdit, QLineEdit *lineEdit2, QLineEdit *lineEdit3, QLineEdit *lineEdit4, QLineEdit *lineEdit5) {
    ...
    // Обновляем данные в таблице article
    query.prepare("UPDATE article SET journal_id = :journal_id, name = :name, rating = :rating, description = :description, price = :cost, release_date = :publication_date "
                  "WHERE id = :id");
    ...
    if (query.exec()) {
        ...
        msgBox.setText("Статья успешно обновлена.");
        msgBox.exec();
        ...
        msgBox.setText("Ошибка при обновлении статьи: " + query.lastError().text());
        msgBox.exec();
    }
}


void updating_article::setFields(const int id_article, const QString &journal, const QString &name, const QString &rating, const QString &description, const QString &price, const QString yearOfFoundation)
{
    article = id_article;
    comboBox->setCurrentText(journal);
    lineEdit->setText(name);
    lineEdit2->setText(rating);
    lineEdit3->setText(description);
    lineEdit4->setText(price);
    lineEdit5->setText(yearOfFoundation);
}
